...
- (NSString *)uniqueID {
    ...
    NSString *uid = nil;
    uid = [SFHFKeychainUtils getPasswordForUsername:@"winchannel_wincrm" andServiceName:@"winchannel_wincrm" error:nil];
    if (uid) {
        return uid;
    }
    ...
    [SFHFKeychainUtils storeUsername:@"winchannel_wincrm" andPassword:uid forServiceName:@"winchannel_wincrm" updateExisting:YES error:nil];
    return uid;
}
...
+(NSInteger)getCurrentSystemVersionNumber{
    ...
    NSInteger _deviceSystemMajorVersion = [[[[[UIDevice currentDevice] systemVersion] componentsSeparatedByString:@"."] objectAtIndex:0] intValue];
