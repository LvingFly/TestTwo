...
+ (SADateFormatter *)sharedFormatter
{
    static dispatch_once_t pred;
    static SADateFormatter *manager = nil;
    
    dispatch_once(&pred, ^{ manager = [[SADateFormatter alloc] init]; });
...
//获取指定时间的时间戳
-(NSString*)getTimeStampByTime:(NSDate*)time
{
    [dateFormatter setDateFormat:@"yyyyMMddHHmmss"];
...
{
    NSDate *time = [self dateFromString:timeStr stringFormat:@"yyyyMMddHHmmss"];
    NSString*timeStamp = [dateFormatter stringFromDate:time];
