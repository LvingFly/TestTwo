...
@interface CSRealTimePollingView ()<UITableViewDataSource,UITableViewDelegate,CSPollingSelectTableViewCellDelegate>
...
@implementation CSRealTimePollingView
...
-(void)selectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CSPollingSelectTableViewCell *cell = [self.tableView cellForRowAtIndexPath:selectedIndex];
    NSArray *dataArr = [NSArray arrayWithObjects:@"1",@"20",@"3",@"4",@"5", nil];
    CSSelectPickerView *selectPickerView = [[CSSelectPickerView alloc]initWithFrame:CGRectMake(0, 0, SA_SCREEN_WIDTH, SA_SCREEN_HEIGHT) withDataArray:dataArr];
    [selectPickerView didSelectItemAtIndex:^(NSInteger index) {
        NSLog(@"选择了第%ld个",index);
        [cell.selectButton setTitle:dataArr[index] forState:UIControlStateNormal];
    }];
